...
export default function Lexicon() {
  ...
  const handleSpeak = (text, langCode, gender) => {
    ...
    if (filteredVoices.length > 0) {
      // Default to the first available voice for that language
      let selectedVoice = filteredVoices[0]; 

      // Strong Matching Logic for David (Male) & Zira/Zara (Female)
      for (let v of filteredVoices) {
        const name = v.name.toLowerCase();
        
        if (gender === 'female') {
          // Check for Windows Zira/Zara, Google Female, or Apple Samantha
          if (name.includes('female') || name.includes('zira') || name.includes('zara') || name.includes('samantha')) {
            selectedVoice = v;
            break; // Stop searching once we find a solid match
          }
        } else if (gender === 'male') {
          // Check for Windows David, Google Male, or Apple Daniel/Alex
          if (name.includes('male') || name.includes('david') || name.includes('alex') || name.includes('daniel')) {
            selectedVoice = v;
            break; // Stop searching once we find a solid match
          }
        }
                        ...
                        <div className="flex items-center gap-2 relative">
                          ...
                          {activeVoiceMenu?.wordId === item.id && activeVoiceMenu?.language === 'eng' && (
                            <div ref={inlineVoiceMenuRef} className="absolute left-full ml-2 z-50 bg-white border border-gray-200 rounded-lg shadow-xl py-1 w-24">
                              <button onClick={() => handleSpeak(activeVoiceMenu.text, activeVoiceMenu.langCode, 'female')} className="w-full text-left px-3 py-1.5 text-xs text-gray-700 hover:bg-gray-100">Female (Zara)</button>
                              <button onClick={() => handleSpeak(activeVoiceMenu.text, activeVoiceMenu.langCode, 'male')} className="w-full text-left px-3 py-1.5 text-xs text-gray-700 hover:bg-gray-100">Male (David)</button>
                            </div>
                          )}
                        </div>
                      </td>

                      {/* KOKBOROK (Spoken via Hindi TTS as closest approximation) */}
